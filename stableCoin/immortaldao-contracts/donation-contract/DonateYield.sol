...
//捐赠sIMMO 到钱包
contract DonateYield is Ownable {
  using SafeMath for uint256;
  using SafeERC20 for IERC20;

  address public immutable sIMMO; // sIMMO contract address
  address public immutable recipient; // Impact Market DAO wallet address
  ...
  function deposit(uint256 _amount) external {
    ...
    require(_amount >= 10000000, "Invalid deposit amount"); //minimum 0.01 sIMMO

    donationInfo[msg.sender].push(
      DonationInfo({ amount: _amount, index: IsIMMO(sIMMO).index() })
    );
  ...
  function withdraw() external {
    ...
    require(totalDeposited[msg.sender] > 0, "No funds to withdraw");

    uint256 totalDepositedByDonor = totalDeposited[msg.sender];
    uint256 totalRebaseYield = getTotalRebaseYield(msg.sender);
  ...
  function getRebaseYield(
    uint256 amount,
    uint256 index,
    uint256 currentIndex
  ) public view returns (uint256) {
    return (amount.mul(currentIndex).div(index)).sub(amount);
  }

  function getTotalRebaseYield(address donor) public view returns (uint256) {
    uint256 totalRebaseYield;
    uint256 currentIndex = IsIMMO(sIMMO).index();
    DonationInfo[] memory donations = donationInfo[donor];

    require(donations.length > 0, "No funds to withdraw");

    for (uint256 i = 0; i < donations.length; i++) {
      totalRebaseYield += getRebaseYield(
        donations[i].amount,
        donations[i].index,
        currentIndex
      );
  ...
  function disableDeposits(bool _active) external onlyManager {
    depositDisabled = _active;
  }

  function disableWithdrawals(bool _active) external onlyManager {
    withdrawDisabled = _active;
  }
}
